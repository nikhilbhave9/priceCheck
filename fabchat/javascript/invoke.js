...
let user, choice, item, price, emailID, proposedinc; // node invoke.js createMsg hello user1 u1@ashoka.edu.in
...
async function main() {
    try {
        ...
        if (choice === 'createItem') {
            await contract.submitTransaction('createItem', item, price, emailID);
            console.log(`${choice} Transaction has been submitted`);
        } else if (choice === 'flagMsg') {
            await contract.submitTransaction('flagMsg', item, proposedinc);
            console.log(`${choice} Transaction has been submitted`);
        } else {
